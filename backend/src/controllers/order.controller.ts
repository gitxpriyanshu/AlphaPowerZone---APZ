...
export const getOrderDetails = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await orderService.getOrderById(req.user?.id || '', id as string, isOwner);
...
export const trackGuestOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!orderId || !email) throw new ApiError(400, 'Order ID and Email are required');
...
export const updateOrderStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await orderService.updateStatus(id as string, status as OrderStatus, note);
  return res.status(200).json(new ApiResponse(200, result, 'Order status updated successfully'));
});

export const cancelOrder = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await orderService.cancelOrder(req.user!.id!, id as string);
