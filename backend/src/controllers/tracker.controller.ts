...
export const logStats = asyncHandler(async (req: Request, res: Response) => {
...
export const getLogs = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await trackerService.getLogs(req.user!.id!, period as string);
  return res.status(200).json(new ApiResponse(200, result, 'Logs fetched successfully'));
});

export const logSupplement = asyncHandler(async (req: Request, res: Response) => {
