...
export const trackerService = {
   ...
   * Log daily stats
   */
  logStats: async (userId: string, data: any) => {
    const profile = await prisma.fitnessProfile.findUnique({ where: { userId } });
    if (!profile) throw new ApiError(404, 'Fitness profile not found');
   ...
   * Get logs for charts
   */
  getLogs: async (userId: string, period = '30d') => {
    const profile = await prisma.fitnessProfile.findUnique({ where: { userId } });
    if (!profile) return [];
   ...
   */
  logSupplement: async (userId: string, name: string, dosage?: string) => {
    const profile = await prisma.fitnessProfile.findUnique({ where: { userId } });
    if (!profile) throw new ApiError(404, 'Fitness profile not found');
