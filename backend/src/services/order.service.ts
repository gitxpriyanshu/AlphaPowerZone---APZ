...
export const orderService = {
   ...
   */
  getOrderById: async (userId: string, orderId: string, isOwner = false) => {
    ...
    if (!isOwner && order.userId !== userId) throw new ApiError(403, 'Unauthorized');
  ...
  trackGuestOrder: async (orderId: string, email: string) => {
    ...
    if (!order) throw new ApiError(404, 'Order not found');
    if (order.user.email !== email) throw new ApiError(403, 'Unauthorized access to this order');
