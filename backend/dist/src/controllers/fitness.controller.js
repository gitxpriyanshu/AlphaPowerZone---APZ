 ...
 * @access  Private
 */
export const analyzeFitness = asyncHandler(async (req, res) => {
    const profileData = req.body;
    const pythonServiceUrl = process.env.PYTHON_AI_SERVICE_URL || 'http://localhost:8000';
    const apiKey = process.env.AI_SERVICE_API_KEY;
    try {
        const response = await axios.post(`${pythonServiceUrl}/fitness/analyze`, profileData, {
            headers: {
                'X-API-Key': apiKey,
        ...
        throw new ApiError(error.response?.status || 500, error.response?.data?.detail || 'Error communicating with AI Service');
    }
});
/**
 ...
 * @access  Private
 */
export const saveFitnessPlan = asyncHandler(async (req, res) => {
